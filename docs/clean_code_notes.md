...
- Name of a variable / function / class, etc - must reveal its intent, if it requires a comment, then it doesn't  convey its intent clearly. 
  ...
  - Class names should be a "noun", and never a "verb". Methods on the otherhand should have "verb". 
  - Say what you mean, and mean what you say. Don't use keywords that relate to something that only a few people will be aware of. Keep it professional. 
  - Pick one word per-concept and stick with it. For instance `get`, `fetch`, and `retrieve` all mean the same. But it doesn't make sense to use each of them for a separate class. 
  - Don't pun, using the same name for 2 different ideas is a pun. Stick to the "one name per concept". 
